...
def get_parser():
    ...
    parser.add_argument("--content_encoder_downsample_size", type=int, default=3,
                        ...
                        指定内容编码器的下采样尺寸，默认值为3。这意味着输入的特征图会被下采样到1/3的大小，控制模型对内容图像的细节提取能力。")
    parser.add_argument("--channel_attn", type=bool, default=True, help="Whether to use the se attention.\
    是否使用通道注意力机制（SE注意力机制），默认为True。SE注意力机制有助于增强模型对重要特征的关注。", )
    ...
    指定负样本的数量。负样本通常用于对比学习或类似的任务中，帮助模型学习区分正负样本的特征。")
    parser.add_argument("--nce_layers", type=str, default='0,1,2,3',
                        help='在对比学习中使用的层。NCE通常指的是噪声对比估计（Noise Contrastive Estimation）')
