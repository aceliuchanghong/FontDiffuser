...
def load_fontdiffuer_pipeline(args):
    ...
    unet.load_state_dict(torch.load(f"{args.ckpt_dir}/unet.pth", weights_only=True))
    style_encoder = build_style_encoder(args=args)
    style_encoder.load_state_dict(
        torch.load(f"{args.ckpt_dir}/style_encoder.pth", weights_only=True)
    )
    content_encoder = build_content_encoder(args=args)
    content_encoder.load_state_dict(
        torch.load(f"{args.ckpt_dir}/content_encoder.pth", weights_only=True)
    )
