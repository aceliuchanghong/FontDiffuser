...
const MAX_NESTED_LIST_ITEMS = 6;  //允许嵌套的列表项的最大层级。
...
const MAX_SENTENCE_LENGTH = 400; //句子的最大长度
...
const LOOKAHEAD_RANGE = 100;  // 寻找句子边界时要向前看的字符数量
...
// 定义了一个辅助函数formatBytes()，用于将字节数转换为更易读的格式（如KB、MB、GB）
...
// Apply the regex 将匹配结果存储在matches变量中
...
if (matches) {
    matches.slice(0, 10).forEach((match, index) => {
        console.log(util.inspect(match, {maxStringLength: 500}));
    });
} else {
